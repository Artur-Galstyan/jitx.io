...
<div class="prose text-justify mx-auto">
    ...
    <section>
        ...
        <p>
            ...
            the number of tokens and <Katex math={"D"} /> is the
            <i>vocabulary size</i>. It can vary, depending on whatever tokeniser
            you used. If you follow along with
...
`}</code
    ...
    <section>
        ...
        <p>
            ...
            the result is scaled down, which also allows for more stable gradients
            later. Typically, the results are scaled down relative to the chosen
            dimensionality. More specifically, the scaling is:
        </p>
        ...
        />
        <p>
            When we apply a softmax afterwords, we turn the attention weights
            into probabilities. At the end, we multiply the attention weights
            (i.e. query @ key) with the value vector. The idea is that the
            attention weights
            <i>highlight</i>
...
`}</code
        ...
        <p>
            ...
            dimension into the number of heads. (Remember, that the input
            dimension of the MHA block is not the same as the original
            dimension, but rather whatever we chose as the embedding dimension
            for the input embeddings.) That way, each head gets the chance to
            learn something new for its dimension, independently of the other
            ...
            feature dimension (4). The reason for that is the "concatenate"
            ...
            <code>qkv_size * n_heads</code> for each of the first 3 linear
            layers? Think of it this way: alternatively, you could have created
            a list of those linear layers and iterated each head one at a time.
...
`}</code
        ...
        <p>
            ...
            />, where <Katex math={"T"} /> is the number of tokens and <Katex
                math={"d"}
            /> is the number of dimensions of the input (which - since the input
            will come <i>after</i> the embedding dimension, will be
            <code>n_embd</code>). However, after applying the linear layers, we
            want the query, key and value matrices to have the shape <Katex
                math={"[T \\times h \\times d_k]"}
            />, where <Katex math={"h"} /> is the number of heads and <Katex
                math={"d_k"}
            /> is equal to <Katex math={"D / h"} />, i.e. the number of
            dimensions of the input divided by the number of heads. This is
            another reason why we wanted the dimension of the input embeddings
            earlier to be the nearest multiple of 64: if we make
            <code>n_heads</code>
...
`}</code
        ...
        <Figure path={"ThT.drawio.svg"} />
        <p>
            And that's why we pass as <code>in_axes = (1, 1)</code> because that
            corresponds to the <Katex math={"h"} /> dimension in <Katex
                math={"T\\times h\\times d_k"}
            />. We specified <code>out_axis = 1</code> because we want to
            "place" the dimension we <code>vmap</code> over into axis 1, which
            means that the output matrix will have the <Katex math={"h"} /> dimension
            on axis 1, which is exactly what we want, since we want the attention
            scores to have the shape of <Katex
                math={"T \\times h \\times T"}
            />. Next up, we apply the scaling and softmax, which is quite
            simple:
        </p>
        <pre><code class="language-python"
                >{`attention_scores = attention_scores / jnp.sqrt(self.qkv_size)
...
`}</code
        ...
        <p>
            ...
            linear layer, which expects the shape to be <Katex
                math={"T \\times d"}
            />, where <Katex math={"d"} /> is the number of dimensions of the input
            (which in turn is the number of output dimensions of the input embeddings).
            As you can see, the "batch dimension" that we want to
            <code>vmap</code>
            over and which is equal for both the scores and <code>v</code> is
            the axis 1, which is <Katex math={"h"} />. To be even more specific,
        ...
        <ol>
            ...
            <li>
                ...
                <code>vmap</code> over axis 1; so for now let's plug that axis out
                (we will place it back in later)
            </li>
            <li>
            ...
            <li>
                The resulting matrix has the shape <Katex
                    math={"T \\times d_k"}
                />
            </li>
            <li>
                If we don't specify the <code>out_axis</code> explicitly, then
                Jax would place it to axis 0, which would give us the result <Katex
                    math={"h \\times T \\times d_k"}
                />. But, we want to plug it back in into axis 1, which would
                make the output matrix have the shape <Katex
                    math={"T \\times h \\times d_k"}
        ...
        <p>
            ...
            be <Katex math={"T"} /> and the -1 specifies to multiply the remaining
            shapes. In our case, the multiplication is <Katex
                math={"h \\cdot d_k"}
            /> and if you remember <Katex math={"d_k = d / h"} />, then you'll
            notice that we end up with our original input dimensionality <Katex
                math={"h \\cdot d_k = D"}
            />:
        </p>
        <pre><code class="language-python"
                >{`output = output.reshape(T, -1)`}</code
            ></pre>
        <p>
            Lastly, passing the output through the output layer, gives us a
            final output matrix with the shape <Katex math={"T \\times d"} />
            And that's it. This was the MHA block for the encoder
            <b>without masking</b>.
        </p>
        <p>
            Let's generalise our implementation a bit and add the masking part.
            But first we need some more background about masking and why we need
            it.
        </p>
    </section>
    <section>
        ...
        <p>
            ...
            also using its own previous output as additional input. This makes
            the decoder <i>autoregressive</i>, i.e. it gets its own output as
            input. During training, the encoder has access to the entire input
            sequence and therefore needs no masking (as it's not
            autoregressive). The decoder on the other hand <b>does</b> need
            masking. Let's take translation as the task at hand here, where we
            have an encoder and a decoder. The goal is to translate from English
            to German. The encoder gets as input sequence "I am happy",
            processes it and generates a <i>summary vector</i>, which the
            decoder gets as input. The decoder's goal is to translate that into
            German <i>Ich bin glücklich</i>. Let's say the input vector for the
            decoder is <Katex math={"[SV, START, ICH]"} />, simplified to <Katex
                math={"[SV, S, I]"}
            />. The current goal is to predict the next word "bin" given the
            summary vector, the start token and the word "ich". Since the input
            is <Katex math={"[SV, START, ICH]"} />, we currently don't even need
            masking yet, as the goal of masking
            <b
                >is to prevent the decoder to have access to future tokens,
                which are the targets itself</b
            >. The decoder computes the self-attention for the word "ich" given
        ...
        />
        <p>
            where the greeks are just some weight parameters. In this example
            the weight <Katex math={"\\gamma"} />should be high, since that is
            very important to predict the next word "bin". However, if the
            training data was instead <Katex math={"[SV, START, ICH, BIN]"} />,
            then, when the decoder computes the attention for "ich", it would be
        </p>
        <Katex
            displayMode
        ...
        />
        <p>
            The decoder's goal is to predict "bin", so it shouldn't know about
            the word "bin". Yet, it computes the attention with <Katex
                math={"\\eta \\cdot BIN"}
            ...
            Consider the mask vector <Katex math={"[1, 1, 1, 0]"} />. If we
            multiply the mask vector with our weight vector <Katex
...
`}</code
            ></pre>
        <p>
            In the code, <code>tril</code> is a triangular matrix, with zeroes
            above its diagonal. With <code>jnp.where</code>, we can generate a <Katex
                math={"T \\times T"}
            /> matrix and change the values to negative infinity, where the
            <code>tril</code>
            matrix had zeroes, i.e. <code>tril == 0</code>. When we softmax
            that, we get a nice probability distribution, that sums to 1.
        </p>
...
`}</code
        ...
        <p>
            ...
            make it a boolean mask directly. Then, we say, that the logits are
            equal to the logits, where the mask is <code>true</code> otherwise it's
            negative infinity. When we softmax that, we get - once again - a nice
            probability distribution.
        </p>
        <p>
            Let's add the second implementation to our code. In fact, I'll just
            show the entire finished MHA implementation:
        </p>
        <pre><code class="language-python"
                >{`
class MultiHeadAttention(eqx.Module):
...
`}</code
            ></pre>
        <p>
            And that's it. That's the MHA block with optional masking. Great!
            Let's add that to our transformer class. You probably forgot about
            that, right? No worries. Here's the current transformer
            implementation:
        </p>
        <pre><code class="language-python"
                >{`class Transformer(eqx.Module):
...
`}</code
    ...
    <section>
        ...
        <p>
            ...
            transformer architecture again, we notice a couple of missing
            things, which are - luckily - very easy to implement. Here it is
            again for reference:
        </p>
        ...
        <p>
            <b
                >For the remainder of this blog post, we will focus on the
                decoder only.</b
            >
...
`}</code
            ></pre>
        <p>
            What's missing now are these <code>add & norm</code> parts. The add
            part is important, because after applying the MHA to the input, the
            input is distorted quite a lot. By adding it back, we effectively
            remind the transformer of how initial structure of the input. As
            your normalisation: it is required, because our network is quite big
            and big networks need normalisation in order to train effectively.
            Here's the RMSNorm implementation which <strike>stole</strike> borrowed
            from the LLaMA2 implementation:
        </p>
        <pre><code class="language-python"
                >{`
...
`}</code
            ></pre>
        <p>
            Let me now show you the full implementation of the (simple)
            transformer:
        </p>
        <pre><code class="language-python"
                >{`
...
`}</code
    ...
    <section>
        ...
        <p>
            ...
            purposes anyway. The following shows a training loop, which uses the
            TinyShakespeare dataloader, which loads the same tiny Shakespeare
            dataset that Andrej used in his tutorial. I won't go into detail
            though, as this is just "regular" engineering and is not specific to
            the transformer.
        </p>
...
`}</code
            ></pre>
        <p>
            If you run this - as is - it won't really be that good. You'll have
            to scale it up by a lot to see <b>real</b> improvements.
            Furthermore, we only have a <b>single</b> MHA block.
            <b>In practice, you would create a sequence of these MHA blocks.</b>
            Like a multi-layer feedforward network, except where each layer is a
            MHA block. That is left to the reader as an exercise.
        </p>
    </section>
    <p>
        And that's it! That was the transformer. I hope you liked it and will
        join me again, on my next engineering blog post. Thanks for reading and
