  ...
  <div class="grid grid-cols-1 gap-x-2 gap-y-1">
    {#each posts as post, i}
      ...
      >
        ...
        {/if}
        <div class="card-body px-4 py-1 my-0 w-full">
      ...
      <div class="divider p-0 m-1 w-1/2 mx-auto" />
    {/each}
  </div>
    ...
    <tbody>
      {#each $page.data.projects as project}
        ...
        >
          ...
          <td class="font-bold text-center md:text-justify">{project.name}</td>

          <td class="table-cell"
            ><a class="link" href={project.link}>Link</a></td
          >
          <td class="hidden md:table-cell">{project.shortDescription}</td>
        </tr>
      {/each}
    </tbody>
  </table>
